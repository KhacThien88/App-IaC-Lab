...
module "set-master-default-router-associate" {
  source = "./Networking/aws_main_route_table_association"
  providers = {
    aws = aws.region-master
  }
...
module "rtb-associate-private-2" {
    ...
    route_table_id = module.route-table-natgateway-2.id
}
module "rtb-associate-private-3" {
    source = "./Networking/aws_route_table_association"
    providers = {
        aws = aws.region-master
    }
    subnet_id = module.subnet_private_3.id
    route_table_id = module.route-table-natgateway-1.id
}
module "rtb-associate-private-4" {
    source = "./Networking/aws_route_table_association"
    providers = {
        aws = aws.region-master
    }
    subnet_id = module.subnet_private_4.id
    route_table_id = module.route-table-natgateway-2.id
}
